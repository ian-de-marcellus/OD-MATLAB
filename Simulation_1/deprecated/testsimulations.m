...
%   Detailed explanation goes here
    ...
    nAgents = 5;
    nZealots = 5;
    random = randi([0 1], nAgents, nAgents);
    agentAdjacency = abs(random - random');
    nStepsAgents = 20;
    nStepsZealots = 30;

    following = randi([0 1], nZealots, nAgents);

    agentOpinions = rand(1, nAgents);
    zealotOpinions = rand(1, nZealots);

    % Discrete agents (w zealots), continuous time

    continuousTimestep = 0.1;
    nStepsZealotsCont = nStepsZealots/continuousTimestep;
    modelC = HKModelContTimeDiscAgentsWithZealots(bound, nAgents, nZealots, agentAdjacency, following, agentOpinions, zealotOpinions, continuousTimestep);

    modelC.simulatesteps(nStepsZealotsCont);
    dataC = modelC.get_data();

    zealotDataContinuous = repmat(zealotOpinions, nStepsZealotsCont, 1);

    dataC = [dataC zealotDataContinuous];


    % Discrete agents (w zealots), discrete time

    model = HKModelDiscTimeDiscAgentsWithZealots(bound, nAgents, nZealots, agentAdjacency, following, agentOpinions, zealotOpinions);

    model.simulatesteps(nStepsZealots);
    data = model.getdata();

    zealotData = repmat(zealotOpinions, nStepsZealots, 1);

    data = [data zealotData];
...
%     model2.writeVideo(1, model2.time);
    ...
    titletext = nAgents + " Agents, " + nZealots + " Zealots, confidence bound: " + bound;
    sgtitle(titletext)

    modelC.setPath("../../Simulation Videos/");
    modelC.videoName = "cont_z_time_sim";
    modelC.writeVideo(1, (modelC.time-1)/continuousTimestep);
